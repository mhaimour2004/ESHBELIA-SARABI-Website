(() => {
  ...
  const updateCategoryRailControls = () => {
    ...
    categoryRailPrev.disabled = options.scrollLeft <= 4;
    categoryRailNext.disabled = options.scrollLeft >= maxScroll - 4;
  };
  const moveCategoryRail = direction => options.scrollBy({left: direction * categoryScrollAmount(), behavior: "smooth"});
  ...
  const moveProduct = offset => {
    ...
    const start = currentProductIndex < 0 ? 0 : currentProductIndex;
    openProduct(products[(start + offset + products.length) % products.length]);
  };
  ...
  dialogImageButton.addEventListener("click", () => {
    ...
    dialogImage.classList.toggle("enlarged");
    if (!dialogImage.classList.contains("enlarged")) dialogImageButton.scrollTo({left:0,top:0,behavior:"smooth"});
    dialogImageHint.textContent = dialogImage.classList.contains("enlarged") ? (language === "ar" ? "اسحب لتحريك الصورة • اضغط للتصغير" : "Drag to move • click to reduce") : (language === "ar" ? "اضغط للتكبير • اسحب لتحريك الصورة" : "Click to enlarge • drag to move");
  });
  dialogImageButton.addEventListener("pointerdown", event => {
    if (!dialogImage.classList.contains("enlarged")) return;
  ...
  const stopImageDrag = event => { if (!dragActive) return; dragActive = false; dialogImageButton.classList.remove("dragging"); if (dialogImageButton.hasPointerCapture(event.pointerId)) dialogImageButton.releasePointerCapture(event.pointerId); };
  ...
  productDialog.addEventListener("close", () => { dialogImage.classList.remove("enlarged"); dialogImageButton.classList.remove("dragging"); dragActive = false; history.replaceState(null, "", "products.html"); });
