(() => {
  ...
  function render() {
    ...
    grid.innerHTML = list.map((p) => `<article class="product-card"><img src="${p.image}" alt="${p.name}" loading="lazy"><div class="product-copy"><div class="product-meta"><span>${p.category}</span><strong>${p.id}</strong></div><h2>${p.name}</h2><span class="status ${p.status.includes("approval") ? "review" : ""}">${p.status}</span><p>${p.summary}</p><ul>${p.specs.map((s) => `<li>${s}</li>`).join("")}</ul><div class="product-actions"><a class="text-link" href="${p.datasheet}" download>Download datasheet ↓</a><a class="text-link" target="_blank" rel="noopener" href="https://wa.me/971565565774?text=${encodeURIComponent(`Hello ESHBELIA SARABI, I would like to enquire about ${p.name} (${p.id}).`)}">WhatsApp enquiry →</a></div></div></article>`).join("") || "<p>No matching products.</p>";
  }
